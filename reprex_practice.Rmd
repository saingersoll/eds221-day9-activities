...
```{r}
# Example 1: 
wrong_mack_creek_lengths <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  select(year:sampledate) %>% 
  filter(section == "CC") %>% 
  # tolower is not necessary because the df was already processed using clean_names, therefore the names are all in snake_case.
  drop_na(species) %>% 
  group_by(species) %>% 
